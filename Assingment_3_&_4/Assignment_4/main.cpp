...
void print(std::vector<std::bitset<8>> ram)
{
    ...
    float cc;
    float dd;
    std::cout<<"Verification of cpu0.s and cpu1.s\n";
    ...
    for(int s = 0; s<array_c.size()-1;s++)
    {
    std::cout<<array_a.at(s)<<"\t"<<"+"<<"\t"<<array_b.at(s)<<"\t"<<"="<<"\t"<<array_c.at(s)<<"\t"<<array_d.at(s)<<"\n";
    }
}

void print_cache(cacheval cv)
{
    std::cout<<"\n";
    std::cout<<"Cachehit 1 "<<cv.cachehit1<<" Cachemiss 1 "<<cv.cachemiss1<<"\n";
    std::cout<<"Cachehit 2 "<<cv.cachehit2<<" Cachemiss 2 "<<cv.cachemiss2<<"\n";
    std::cout<<"Cachehit 3 "<<cv.cachehit3<<" Cachemiss 3 "<<cv.cachemiss3<<"\n";
    std::cout<<"Cachehit 4 "<<cv.cachehit4<<" Cachemiss 4 "<<cv.cachemiss4<<"\n";
    std::cout<<"\n";
}
...
int main()
{
    ...
    while(cnt != 2)
    {
        Event e(i);
        Event e2(i);
        cnt2 = e.process(cpu0.pip,cpu0.pc,cpu0.integer_reg,cpu0.float_reg,po,i,cno0,cv,fc1);
        cnt = e2.process(cpu1.pip,cpu1.pc,cpu1.integer_reg,cpu1.float_reg,po,j,cno1,cv,fc2);
        s.schedule(e,i);
        s.schedule(e2,j);
        ss++;
        std::cout<<"\n";
