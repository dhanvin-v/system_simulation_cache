...
int Event::process(std::vector<Pipeline> &pip, int &pc, std::vector<std::bitset<32>> &integer_reg, std::vector<std::bitset<32>> &float_reg, port &po, int &tickval, int &fc)
{
    int flag = 0, cnt = 0, slag = 0,count = 0,dlag = 0, clag = 0, glag = 0, hlag = 0, currtime = 0, maxtime = 0, schedule = 0;
    ...
    if(pip.at(0).stage == "execute" && (clag == 0) && (glag == 0))
    {
    ...
    {    
        for(int i = 0;i<pip.size(); i++)
        {
            ...
            {
                flag = execute(po, pip, pc, integer_reg, float_reg, currtime);
                fc++;
                if(currtime > maxtime)
                maxtime = currtime;
                currtime = 0;
...
int Event::execute(port &po, std::vector<Pipeline> &pip, int &pc, std::vector<std::bitset<32>> &integer_reg, std::vector<std::bitset<32>> &float_reg, int &tickval)
{
...
void Event::fetch(port &po , std::vector<Pipeline> &pip, int &pc, int &tickval)
{
    packet ps;
    ps.type = "fetch";
...
//Constructor of CPU class used to create empty spaces
CPU::CPU(int sp_addr)
{
    for(int i = 0;i<32;i++)
    {
