...
//Constructor for packet used to create empty spaces
packet::packet()
{
    for(int i = 0;i<4;i++)
    {
...
//Function used to assign ram reference
...
void membus::sendPacket(packet &p, int &tickval)
{
    ...
    {
        //check_memstate(tickval);
        check_prevaddr(p,tickval);
        ...
        else if((p.addr_valls.at(0) >= 0x400) && (p.addr_valls.at(0) <=0xBFF))
        {
            get_D_port_cpu(p);
